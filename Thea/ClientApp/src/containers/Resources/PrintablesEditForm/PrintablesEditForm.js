...
import './PrintablesEditForm.css';

export class PrintablesEditForm extends Component {
    displayName = PrintablesEditForm.name;
    constructor(props) {
        super(props);
        this.state = {
            id: 0,
    ...
    beginEdit = (index) => {
        this.setState({
            id: this.state.printList[index].id,
            title: this.state.printList[index].title,
            srcURL: this.state.printList[index].srcURL,
            showInputs: true,
            crudType: "Edit"
        });
    }
    ...
    addData() {
        ...
        axios.post(window.location.origin + "/api/printables", fd, headers).then(resp => {
            this.props.fillState();
            this.fillState();
            document.getElementById("popup-container").scrollTop = 0;
            this.setState({
    ...
    editData() {
        ...
        axios.put(window.location.origin + "/api/printables/" + this.state.id, fd, headers).then(resp => {
            document.getElementById("popup-container").scrollTop = 0;
            this.setState({
                showInputs: false,
                title: '',
    ...
    deleteData(id) {
        ...
        axios.delete(window.location.origin + "/api/printables/" + id, headers).then(resp => {
            this.props.fillState();
            this.fillState();
    ...
    render() {
        ...
        let buttons = null;
        this.state.printList.map((tr, index) => {
            tableRows.push(
                <tr key={index}>
                    <td>{tr.title}</td>
